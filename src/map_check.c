...
int init_fc_color(t_cub3d *data, char *str)
{
	...
	if (!ft_strncmp(str, "F ", 2))
	{
		...
		while (data->texture->floor[i])
		{
			...
			{
				ft_error(data, "invalid floor color\n");
			}
			i++;
		}
		i = 0;
	...
	else if (!ft_strncmp(str, "C ", 2))
	{
		...
		while (data->texture->ceiling[i])
		{
			...
			{
				ft_error(data, "invalid ceiling color\n");
			}
			i++;
		}
	}
...
int	init_file_data(t_cub3d *data, char **array)
{
	...
	while (array[++i])
	{
		if (!ft_strncmp(array[i], "\r", 1) || init_textures(data, array[i]) == 0 || init_fc_color(data, array[i]) == 0)
			;
		else if (!ft_strncmp(array[i], " ", 1) || !ft_strncmp(array[i], "1", 1))
		{
			...
			ft_error(data, "invalid file format\n");
	}
	return (0);
}
...
void	handle_map(t_cub3d *data)
{
	...
	while (data->map[row])
	{
		if (ft_strncmp(data->map[row], "\r", 1) == 0)
			ft_error(data, "empty line found in map.");
		row++;
	}
}
